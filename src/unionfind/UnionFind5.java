...
public class UnionFind5 implements UnionFind {
    ...
    private int find(int index) {
        ...
        // 不断向上寻找父节点，直到找到根顶点为止
        while (!isRoot(index)) {
            // 在寻找的过程中，进行路径压缩，将 index 处的元素指向其父节点的父节点，从而不断降低树的高度
            parentId[index] = parentId[parentId[index]]; // 在此处，树的高度会发生变化，但是出于性能考虑，不更新 rank（rank 只是一个估计值）
